...
describe("dnd al recorder - User and Character classes", () => {
    ...
    describe("Character class", () => {
        ...
        describe("spendGold - method that removes gold from character", () => {
            ...
            it("spendGold - alerts user if there is not enough money to spend and does not carry out action", () => {
                const newChar = new Character("Greqial Klaqa Rikkaarr", "Circle of Moon Druid", 10, 30, 6000)
                const logSpy = jest.spyOn(global.console, "log")
                newChar.spendGold(7000)
                expect(logSpy).toHaveBeenCalledWith("Greqial Klaqa Rikkaarr does not have enough money for that!")
                expect(newChar.gold).toBe(6000)
                
            })
            it("spendGold - alerts user if no input amount entered or invalid data type", () => {
                const newChar = new Character("Greqial Klaqa Rikkaarr", "Circle of Moon Druid", 10, 30, 6000)
                const logSpy = jest.spyOn(global.console, "log")
                newChar.spendGold()
                expect(logSpy).toHaveBeenCalledWith("You need to specify how much gold to spend!")
                newChar.spendGold("23")
                expect(logSpy).toHaveBeenCalledWith("You must enter a number for the amount of gold to spend!")
            })
            it("spendGold - gives user message detailing changes in amounbt of gold", () => {
                const newChar = new Character("Greqial Klaqa Rikkaarr", "Circle of Moon Druid", 10, 30, 6000)
                const logSpy = jest.spyOn(global.console, "log")
                newChar.spendGold(1000)
                expect(newChar.gold).toBe(5000)
                expect(logSpy).toHaveBeenCalledWith("Greqial Klaqa Rikkaarr had 6000 gold, spent 1000 gold and now has 5000 gold")
            })
        })
    })
})